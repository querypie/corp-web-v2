...
- Playwright 기반 브라우저 E2E는 로컬 전용입니다.
- 현재 `contact-us` stage 검증 명령은 `npm run e2e:local:contact-us:stage` 입니다.
- 설정과 실행 방법은 [Local-only Playwright E2E](docs/reference/local-e2e.md)를 따릅니다.
...
- [Local-only Playwright E2E](docs/reference/local-e2e.md) — stage 기반 로컬 전용 contact-us 브라우저 E2E 실행 방법과 환경변수
