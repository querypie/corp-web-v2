...
describe("GET /api/downloads/file", () => {
  ...
  it("demo PDF는 읽어서 attachment 응답을 반환한다", async () => {
    ...
    const request = new Request(
      "http://localhost/api/downloads/file?src=/demo/aip-features/sample.pdf&fileName=sample.pdf",
    );
    const response = await GET(request);

    expect(response.status).toBe(200);
    expect(mockReadFile).toHaveBeenCalledWith(
      path.join(process.cwd(), "public", "demo", "aip-features", "sample.pdf"),
    );
  });

  it("허용되지 않은 public 경로는 404를 반환한다", async () => {
    const request = new Request(
      "http://localhost/api/downloads/file?src=/path/assets/pages/platforms/acp/acp-integration.pdf&fileName=sample.pdf",
    );
    const response = await GET(request);
