...
import DocsListClientPage from "@/components/pages/resources/ResourcesListClientPage";
...
export default async function ResourcesPage({ params, searchParams }: DocsPageProps) {
  ...
  if (locale !== "ja" && legacyCategory) {
    permanentRedirect(getPublicListHref("resources", locale, legacyCategory));
  }
  ...
  const publicDocsItems = (await readContentState("resources", { includeBodies: false }))
  ...
  const fallbackItems = docsItems.map((item) => ({
    ...
    href: item.contentType === "outlink" ? item.externalUrl : getPublicDetailHref("resources", locale, item.id, item.categorySlug),
...
export async function generateMetadata({ params, searchParams }: DocsPageProps): Promise<Metadata> {
  ...
  const { metadataDescription, metadataTitle } = getResourcesPageCopy(locale);
  const selectedCategory: DocsCategorySlug =
  ...
  return withDynamicOgImage({
    ...
    alternates: {
      canonical: getPublicListHref("resources", locale, selectedCategory),
    },
