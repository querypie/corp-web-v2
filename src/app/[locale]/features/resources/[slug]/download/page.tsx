...
export default async function WhitePaperDownloadRoute({ params }: WhitePaperDownloadRouteProps) {
  ...
  return (
    <ContentDownloadPage
      ...
      returnUrl={getPublicDetailHref("resources", locale, resolvedSlug, currentEntry.categorySlug)}
      section="resources"
...
export async function generateMetadata({ params }: WhitePaperDownloadRouteProps): Promise<Metadata> {
  ...
  return withDynamicOgImage({
    ...
    description,
    alternates: {
      canonical: `${getPublicDetailHref("resources", locale, resolvedSlug, currentEntry.categorySlug)}/download`,
    },
