...
import { getWebinarDemoHref, resolveWebinarDemoRoute } from "@/features/demo/webinar";

type Props = {
  params: Promise<{ locale: string; id: string; rest?: string[] }>;
};

export default async function WebinarDemoDetailRoute({ params }: Props) {
  const { locale, id, rest } = await params;

  if (!isLocale(locale)) notFound();

  const route = resolveWebinarDemoRoute(locale as Locale, id, rest);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: getLocalizedContent(currentEntry.title, locale as Locale),
    alternates: {
      canonical: getWebinarDemoHref(locale as Locale, id) ?? undefined,
    },
  };
}
