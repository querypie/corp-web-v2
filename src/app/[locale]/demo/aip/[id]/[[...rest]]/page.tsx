...
import { getAipDemoHref, resolveAipDemoRoute } from "@/features/demo/aip";

type Props = {
  params: Promise<{ locale: string; id: string; rest?: string[] }>;
};

const contentId = "google-oauth-demo";

export default async function AipDemoDetailRoute({ params }: Props) {
  const { locale, id, rest } = await params;

  if (!isLocale(locale)) notFound();

  const route = resolveAipDemoRoute(locale as Locale, id, rest);
  ...
  const [allDemoItems, currentEntry] = await Promise.all([
    ...
    readContentItem("demo", contentId, { includeBodies: true }),
  ]);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const currentEntry = await readContentItem("demo", contentId, { includeBodies: false });
  if (!currentEntry || !isPublishedContentAccessible(currentEntry)) return {};

  return {
    title: getLocalizedContent(currentEntry.title, locale as Locale),
    alternates: {
      canonical: getAipDemoHref(locale as Locale, id) ?? undefined,
    },
  };
}
