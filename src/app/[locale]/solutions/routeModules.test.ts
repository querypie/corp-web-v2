...
import { metadataByLocale as Metadata1 } from "./aip/page";
...
import { metadataByLocale as Metadata2 } from "./aip/usage-based-llm/page";
...
import { metadataByLocale as Metadata3 } from "./aip/mcp-gateway/page";
...
import { metadataByLocale as Metadata4 } from "./aip/fde-services/page";
...
import { metadataByLocale as Metadata5 } from "./aip/integrations/page";
...
import { metadataByLocale as Metadata6 } from "./acp/page";
...
import { metadataByLocale as Metadata7 } from "./acp/database-access-controller/page";
...
import { metadataByLocale as Metadata8 } from "./acp/system-access-controller/page";
...
import { metadataByLocale as Metadata9 } from "./acp/kubernetes-access-controller/page";
...
import { metadataByLocale as Metadata10 } from "./acp/web-access-controller/page";
...
import { metadataByLocale as Metadata11 } from "./acp/integrations/page";
...
describe("solutions route-local modules", () => {
  ...
  it("모든 canonical solution entry에 대해 route-local metadata를 제공한다", () => {
    for (const entry of solutionEntries) {
      const modules = routeModules[entry.id as keyof typeof routeModules];
      expect(modules.metadata.en, `${entry.id} en`).toMatchObject({ title: expect.any(String), description: expect.any(String) });
      expect(modules.metadata.ko, `${entry.id} ko`).toMatchObject({ title: expect.any(String), description: expect.any(String) });
      expect(modules.metadata.ja, `${entry.id} ja`).toMatchObject({ title: expect.any(String), description: expect.any(String) });
    }
  });

  it("page generateMetadata가 route-local metadata와 canonical href를 사용한다", async () => {
    for (const entry of solutionEntries) {
      const modules = routeModules[entry.id as keyof typeof routeModules];
      await expect(modules.generateMetadata({ params: Promise.resolve({ locale: "ko" }) })).resolves.toMatchObject({
        title: modules.metadata.ko.title,
        description: modules.metadata.ko.description,
