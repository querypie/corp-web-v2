...
export default async function WhitePaperListPage({ params, searchParams }: Props) {
  ...
  const allItems = await loadMdxListItems("white-paper", locale);
  const paginated = paginateItems(allItems, requestedPage, MDX_LIST_PAGE_SIZE);
  const basePath = getLocalePath(locale, "/white-papers");
  const copy = getMdxListPageCopy("white-paper", locale);
