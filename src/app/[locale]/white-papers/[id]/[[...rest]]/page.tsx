...
import WhitePaperLayout from "@/components/mdx-layout/WhitePaperLayout";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const route = resolveMdxDetailRoute("white-paper", locale as Locale, id, rest, frontmatter.slug, frontmatter.title);
...
export default async function WhitepaperMdxPage({ params }: Props) {
  ...
  return (
    <WhitePaperLayout frontmatter={frontmatter} headings={headings} locale={locale as Locale}>
      {content}
    </WhitePaperLayout>
  );
}
