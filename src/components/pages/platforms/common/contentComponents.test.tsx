...
import { buildPlatformContentComponents } from "./contentComponents";

function getComponents() {
  return buildPlatformContentComponents({ locale: "ko", searchParams: { category: "db" } }) as any;
}

describe("buildPlatformContentComponents", () => {
  it("내부 Link href에 locale prefix를 붙인다", () => {
    const { Link } = getComponents();

    render(<Link href="/platforms/aip">AIP</Link>);
  ...
  it("Integrations는 현재 category에 맞는 product만 노출한다", () => {
    ...
    render(
      <Integrations
        allLabel="All"
        basePath="/platforms/acp"
        categories={[
          { id: "db", label: "Databases" },
          { id: "cloud", label: "Cloud" },
        ]}
        iconBasePath="/assets/pages/platforms/acp/integrations"
        products={[
    ...
    expect(screen.getByRole("link", { name: "Databases (1)" })).toHaveAttribute(
      "href",
      "/ko/platforms/acp?category=db",
    );
    expect(screen.getByRole("img", { name: "PostgreSQL" })).toHaveAttribute(
      "src",
      "/assets/pages/platforms/acp/integrations/postgresql-icon.svg",
    );
  });
      ...
      <KillerFeatures title="ACP Features">
        <KillerFeatureCategory label="Database Access Control">
          <KillerFeature
            ...
            image="public/assets/pages/platforms/acp/dac3.gif"
          >
