...
export function buildPlatformContentComponents({
  locale,
  searchParams,
}: {
  locale: Locale;
  ...
  return {
      ...
      return (
        <Component
          ...
          className={joinClasses(
            background === "gray" && "bg-bg-deep",
            background === "dac" && "bg-[var(--color-info-surface)]",
    ...
    StaticH1: ({ children }: { children?: ReactNode }) => (
      <h1 className="text-balance text-4xl font-semibold tracking-tight text-fg md:text-5xl">
        {children}
      </h1>
    ),
    ...
    StaticH4: ({ children }: { children?: ReactNode }) => (
      <h3 className="text-balance text-2xl font-semibold tracking-tight text-fg md:text-3xl">
        {children}
      </h3>
    ),
    ...
    StaticHeader: ({
      ...
      <div className="max-w-4xl text-lg leading-8 text-mute" style={localizedTextColor(color)}>
        {children}
      </div>
    ),
    ...
    StaticBody: ({
      ...
      <div className={joinClasses("text-base leading-7 text-mute", className)} style={localizedTextColor(color)}>
        {children}
      </div>
    ),
    ...
    ThumbnailYoutube: ({
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="group relative block overflow-hidden rounded-3xl border border-border bg-[var(--color-inverse-bg)] shadow-sm"
      >
        <img
        ...
        <span className="absolute inset-0 flex items-center justify-center">
          <span className="rounded-full bg-[rgb(255_255_255/0.92)] px-5 py-3 text-sm font-semibold text-[var(--color-inverse-bg)] shadow-sm">
            Play video
          </span>
        </span>
      </a>
    ...
    Youtube: ({ src, title }: { src?: string; title?: string }) => (
      <div className="aspect-video overflow-hidden rounded-3xl border border-border shadow-sm">
        <iframe
          src={src}
    ...
    MainFeatureDescription: ({
      ...
      const media = (
        <div className={joinClasses("overflow-hidden rounded-3xl border border-border bg-bg", imageShadow && "shadow-xl shadow-[rgb(var(--color-shadow-rgb)/0.1)]")}>
      ...
      const text = (
        <div className="flex flex-col gap-5">
          ...
          <div className="text-base leading-7 text-mute">{renderWithLineBreaks(description)}</div>
          {checkList.length > 0 ? (
            <ul className="flex flex-col gap-3">
              {checkList.map((item) => (
                <li key={item} className="flex items-start gap-3 text-base leading-7 text-mute">
                  <span className="mt-1 inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary text-xs text-bg">
                    ✓
                  </span>
                  <span>{item}</span>
                </li>
    ...
    IntroducingQueryPie: ({
      ...
      <section className="bg-[var(--color-inverse-bg)] text-[var(--color-inverse-fg)]">
        <div className="mx-auto flex max-w-[1200px] flex-col gap-10 px-6 py-16 md:px-10 md:py-20">
          <div className="flex max-w-3xl flex-col gap-4">
            ...
            <p className="text-base leading-7 text-[var(--color-inverse-muted)]">{renderWithLineBreaks(description)}</p>
          </div>
          <ul className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            {items.map((item) => (
              <li key={item.title} className="flex h-full flex-col gap-5 rounded-3xl border border-[var(--color-inverse-border)] bg-[rgb(255_255_255/0.05)] p-6">
                ...
                <div className="flex flex-1 flex-col gap-4">
                  ...
                  {item.subTitle ? (
                    <span className="inline-flex w-fit rounded-full bg-[rgb(255_255_255/0.1)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--color-inverse-fg)]">
                      {renderWithLineBreaks(item.subTitle)}
                    </span>
                  ) : null}
                  <p className="text-sm leading-7 text-[var(--color-inverse-muted)]">{renderWithLineBreaks(item.description)}</p>
                </div>
        ...
        <div className="flex flex-col gap-8">
              ...
              <a
                ...
                className={joinClasses(
                  ...
                  currentCategory === "all"
                    ...
                    : "border-border bg-bg text-fg hover:border-border-strong hover:bg-bg-content",
                )}
              >
          ...
          <ul className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {visibleProducts.map((product) => (
              <li key={product.label} className="flex min-h-[148px] flex-col items-center justify-center gap-4 rounded-3xl border border-border bg-bg px-5 py-6 text-center shadow-sm">
                <img
                  alt={product.label}
                  src={`${iconBasePath}/${product.svgFilename}.svg`}
                  className="h-16 w-16 object-contain"
                />
                <span className="text-sm font-medium leading-6 text-fg">{product.label}</span>
              </li>
            ))}
          </ul>
        </div>
    ...
    KeyFeature: ({
      ...
      <li className="flex h-full flex-col gap-4 rounded-3xl border border-border bg-bg p-6 shadow-sm">
        <img alt={label} src={toPublicSrc(iconFilepath)} className="h-10 w-10" />
        <h4 className="text-lg font-semibold text-fg">{renderWithLineBreaks(label)}</h4>
        <p className="text-sm leading-7 text-mute">{renderWithLineBreaks(description)}</p>
      </li>
    ),
    ...
    DarkBadge: ({ children }: { children?: ReactNode }) => (
      <span className="inline-flex rounded-full bg-primary px-3 py-1 text-xs font-semibold uppercase tracking-wide text-bg">
        {children}
      </span>
    ),
            ...
            <div className="flex flex-col gap-12">
              {categories.map((category) => (
                <section key={category.label} className="flex flex-col gap-5">
                  <h3 className="text-xl font-semibold text-fg md:text-2xl">{category.label}</h3>
                  <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
                    {category.features.map((feature) => (
                      <article
                        key={`${category.label}-${feature.title}`}
                        className="flex h-full flex-col gap-5 overflow-hidden rounded-3xl border border-border bg-bg p-6 shadow-sm"
                      >
                        <img alt={feature.title} src={toPublicSrc(feature.image)} className="h-auto w-full rounded-2xl" />
                        <div className="flex flex-1 flex-col gap-4">
                          <h4 className="text-2xl font-semibold tracking-tight text-fg">
    ...
    p: ({ children }: { children?: ReactNode }) => (
      <p className="text-base leading-7 text-mute">{children}</p>
    ),
  };
}
