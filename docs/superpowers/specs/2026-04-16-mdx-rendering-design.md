...
- `src/content/mdx/**`의 본문 단락은 모두 **한 줄에 한 문장(one sentence per line)** 형식으로 유지한다.
- 문단 안에서 새 문장을 추가할 때는 같은 단락이어도 문장마다 줄바꿈한다.
- 리스트 항목과 JSX 기반 본문 텍스트(`Table.Td`, `InfoNote`, 일반 텍스트 children 포함)도 여러 문장을 한 줄에 합치지 않는다.
