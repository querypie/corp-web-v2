...
현재 채팅 UI는 `/api/ai-chat`에서 공식 문서 일부를 검색합니다. 로컬에서는 서버가 사내 모델을 호출하고, Vercel Preview의 키 없는 사내 모델 설정에서는 직원의 브라우저가 모델을 직접 호출합니다. 문서는 수동 수집한 스냅샷이며, 전체 사이트를 실시간 검색하거나 임베딩 DB를 운영하는 구성은 아닙니다.
...
로컬 서버는 `POST /v1/chat/completions`를 호출합니다. Vercel에서 사내 LLM으로 직접 호출하면 연결 오류가 발생하므로 Preview에서는 브라우저에 공개 근거와 모델 요청을 전달합니다. 직원의 브라우저에서 사내 LLM에 접속할 수 있어야 하며, 사내 서버가 해당 Preview Origin의 CORS를 허용해야 합니다. 현재 주소의 CORS 허용을 확인했습니다.

브라우저 호출은 일반 Preview + 지정된 사내 주소 + API key 없음 조건에서만 사용합니다. 인증 키가 설정되거나 서버 주소가 바뀌면 서버 호출을 사용하며, 인증 정보는 브라우저에 보내지 않습니다. Preview 이외에서는 `AI_CHAT_ENABLED=true`로 명시적으로 활성화해야 합니다. CMS의 localhost 외 접근 차단 정책은 별도로 유지됩니다.
...
- 테스트용 제한은 서버 API 프로세스당 분당 30회, 동시 3회입니다. Preview에서는 근거 준비 요청에 적용되며, 브라우저에서 모델로 보내는 요청의 제한은 사내 서버 정책을 따릅니다. 다중 인스턴스에 적용되는 운영용 인증·사용량 제한을 대체하지 않습니다.
