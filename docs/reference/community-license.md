...
4. **라이선스 발급** (`issueLicense`) — `DESKPIE_API_BASE_URL`, `DESKPIE_API_KEY` 미설정 시 skip; 설정된 경우 `201` 응답은 `licenseId`를 반환하며 validation/business 실패는 non-2xx로 전체 흐름 중단
...
| 변수 | 필수 | 설명 |
|------|------|------|
| `DESKPIE_API_BASE_URL` | 선택 | DeskPie public API base URL. 미설정 시 발급 단계 skip |
| `DESKPIE_API_KEY` | 선택 | DeskPie public API key. 미설정 시 발급 단계 skip |
